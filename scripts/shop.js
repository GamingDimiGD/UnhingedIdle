...
let lightbulb = new ShopItem('電燈', 10, '燈泡散發出的光可以被轉成少量的星塵', 'lightbulb', 5, 0, () => giveAch('lightbulb'))
...
let electricity = new ShopItem('繳電費', 1e3, '繳電費來讓政府給你用電', 'electricity', 200, 0, () => giveAch('government'))
...
let windTurbine = new ShopItem('風力發電機', 1e9, '建造超大風車', 'windTurbine', 1e6, 0, () => giveAch('windTurbine'))

let buildMoreBuildings = new ShopItem('建造分廠', 1e12, '設立分廠一起製造星塵', 'bmb', 1e8, 0, () => giveAch('buildMore'))

let hire = new ShopItem('申請人才', 1e15, '申請人才，讓他們想辦法增加產量', 'hire', 1e10)

let rocket = new ShopItem('發射火箭', 1e19, '不但可以使用它發出的光，還可以研究別顆星球，一舉兩得!', 'rocket', 1e12)

let buildOnOtherPlanets = new ShopItem('設立外星工廠', 1e24, '在其他星球設廠', 'boop', 1e16)

let buyStars = new ShopItem('購買恆星', 1e30, '把恆星買下來，採收其能量來製造星塵', 'buyStars', 1e19, 0, () => giveAch('buyStar'))

let portal = new ShopItem('異界傳送門', 1e35, '可以去異界採收能量並建造工廠製造閃', 'portal', 1e22)

let alienWorker = new ShopItem('申請異界人與外星人', 1e40, '聘請他們來幫忙研究製造閃的方式', 'alienWorker', 1e25)

let alienTech = new ShopItem('異界與外星科技', 1e45, '使用它們的科技，大量增加收入', 'alienTech', 1e29)

let manipulateLuck = new ShopItem('控制運氣', 1e50, '可使自己的運氣變好，機器不壞掉', 'manipulateLuck', 1e33)

let timeMachine = new ShopItem('時光機', 1e60, '可以穿越時空至古代與未來，使用恐龍與未來科技來製造閃', 'timeMachine', 1e37)

let newUniverse = new ShopItem('創造新次元', 1e70, '可以製造一個新的次元來增加產量 (圖片為新次元的大霹靂)', 'newUniverse', 1e42)

let antimatterEnergy = new ShopItem('反物質能量', 1e80, '使用反物質能量來製造能量', 'antimatterEnergy', 1e47)

let breakReality = new ShopItem('突破、超越現實的工廠', 1e100, '使用突破、超越現實的方式賺閃', 'breakReality', 1e53)

let breakThe4thWall = new ShopItem('突破第四面牆', 1e120, '"我知道你在玩這個遊戲，你的電腦的能量可以提供我能量賺閃"', 'breakThe4thWall', 1e60)
